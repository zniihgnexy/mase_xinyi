...
def insert_normalization_after_relu(graph):
    for node in list(graph.fx_graph.nodes):
        if node.op == 'call_module' and isinstance(graph.modules[node.target], nn.ReLU):
            norm_layer = nn.BatchNorm1d(num_features=...)

            norm_layer_name = f'{node.name}_norm'
            setattr(graph.model, norm_layer_name, norm_layer)
            graph.modules[norm_layer_name] = norm_layer

            with graph.fx_graph.inserting_after(node):
                new_node = graph.fx_graph.call_module(norm_layer_name, args=(node,))
            
            new_node.meta['mase'] = MaseMetadata(node=new_node, model=graph.model)
...
#########################################################################################################
### run another analysis pass, profile statistics analysis pass
# new args
# pass_args = {
#     "by": "type",                                                            # collect statistics by node name
#     "target_weight_nodes": ["linear"],                                       # collect weight statistics for linear layers
...
# # report_node_meta_param_analysis_pass: prints out the node meta param
...
# print("finish the second pass")

# #########################################################################################################

# print("node after profile statistics analysis pass")
# for node in ori_mg.fx_graph.nodes:
#     print(node)
